...
public:
    MemoryBackend(
        ...
        : name_(get(keyValues, "path")), journal_(journal), factory_(factory)
    {
...
MemoryFactory::createInstance(
    ...
    beast::Journal journal)
{
    return std::make_unique<MemoryBackend>(keyBytes, keyValues, journal, *this);
}

MemoryDB&
MemoryFactory::open(std::string const& path)
{

...
TEST_SUITE_BEGIN("JobQueue");

TEST_CASE("add job")
{
    auto globals = Globals::make();
    auto& jQueue = globals->jobQueue();
    {
        // addJob() should run the Job (and return true).
        std::atomic<bool> jobRan{false};
        CHECK(jQueue.addJob(jtCLIENT, "JobAddTest1", [&jobRan]() {
            jobRan = true;
        }) == true);
    ...
    }
    {
        ...
        bool unprotected;
        CHECK(jQueue.addJob(jtCLIENT, "JobAddTest2", [&unprotected]() {
            unprotected = false;
        }) == false);
    }
}

TEST_CASE("post coroutine")
{
    auto globals = Globals::make();
    auto& jQueue = globals->jobQueue();
    {
